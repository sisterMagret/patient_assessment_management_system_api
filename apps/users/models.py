...
from apps.utils.enums import (AuthTokenEnum, AuthTokenStatusEnum, 
                               BloodGroupType, GenderType,
                               UserType, Genotype, ValidIDType)
...
class User(AbstractUser, AbstractUUID):
    ...
    Custom User Model
    """
    user_role = models.PositiveSmallIntegerField(default=UserType.USER, choices=UserType.choices())
    ...
    gender = models.CharField(choices=GenderType.choices(), max_length=255, null=True, blank=True)
...
class Patient(AbstractUUID):
    ...
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name="patient_user", null=True, blank=True)
    blood_group = models.CharField(max_length=3, choices=BloodGroupType.choices(), null=True, blank=True)
    genotype = models.CharField(max_length=3, choices=Genotype.choices(), null=True, blank=True)
    ...
    emergency_contact = models.ForeignKey(EmergencyContact, null=True, related_name="emergency_contact", blank=True, on_delete=models.SET_NULL)
...
class Practitioner(AbstractUUID):
    ...
    specializations = models.ManyToManyField(PractitionerSpecialization, related_name='practitioner_specializations')
...
class AuthToken(AbstractUUID):
    ...
    type = models.PositiveSmallIntegerField(choices=AuthTokenEnum.choices(), default=AuthTokenEnum.VERIFICATION)
    token = models.CharField(max_length=255, null=True, blank=True)
    status = models.PositiveSmallIntegerField(choices=AuthTokenStatusEnum.choices(), default=AuthTokenStatusEnum.PENDING, editable=False)
