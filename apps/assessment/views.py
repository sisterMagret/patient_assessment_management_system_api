...
class AssessmentViewSet(BaseViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            instance = self.get_object()
            data = self.get_data(request)
            serializer = CreateAssessmentSerializer(instance, data=data, partial=True)
            if serializer.is_valid():
                assessment = serializer.save()
                context.update({
                    "data": self.serializer_class(assessment).data,
    ...
    def list_questions(self, request, *args, **kwargs):
        ...
        try:
            ...
            context.update({"data": QuestionSerializer(questions, many=True).data})
        except Exception as ex:
            context.update({"status": status.HTTP_400_BAD_REQUEST, "message": str(ex)})
        return Response(context, status=context["status"])
    ...
    def add_question(self, request, *args, **kwargs):
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                context.update({"data": serializer.data, "message": "Question added successfully"})
            else:
                context.update({
                    "errors": serializer.errors,
                    "status": status.HTTP_400_BAD_REQUEST
    ...
    def list_assessment_types(self, request, *args, **kwargs):
        ...
        try:
            ...
            context.update({"data": AssessmentTypeSerializer(assessment_types, many=True).data})
        except Exception as ex:
            context.update({"status": status.HTTP_400_BAD_REQUEST, "message": str(ex)})
        return Response(context, status=context["status"])
    ...
    @action(detail=True, methods=["delete"], url_path="types/(?P<type_id>[^/.]+)", description="Delete an assessment type")
    def delete_assessment_type(self, request, type_id=None, *args, **kwargs):
        """ Delete an assessment type """
        context = {"status": status.HTTP_204_NO_CONTENT}
        try:
            assessment_type = AssessmentType.objects.get(id=type_id)
            assessment_type.delete()
            context.update({"message": "Assessment type deleted successfully"})
        except AssessmentType.DoesNotExist:
            context.update({"status": status.HTTP_404_NOT_FOUND, "message": "Assessment type not found"})
        except Exception as ex:
            context.update({"status": status.HTTP_400_BAD_REQUEST, "message": str(ex)})
        return Response(context, status=context["status"])
    ...
    @swagger_auto_schema(
        ...
        responses={200: openapi.Response("Success", AssessmentTypeSerializer)}
    )
    @action(detail=True, methods=["put"], url_path="types/(?P<type_id>[^/.]+)", description="Update an assessment type")
    def update_assessment_type(self, request, type_id=None, *args, **kwargs):
    ...
    @action(detail=True, methods=["get"], url_path="questions-and-answers", description="List all questions and answers for an assessment")
    ...
    @action(detail=True, methods=["post"], url_path="questions/(?P<question_id>[^/.]+)/answer", description="Submit an answer for a question")
    def submit_answer(self, request, question_id=None, *args, **kwargs):
        ...
        try:
            ...
            context.update({"status": status.HTTP_404_NOT_FOUND, "message": "Question not found"})
        except Exception as ex:
            context.update({"status": status.HTTP_400_BAD_REQUEST, "message": str(ex)})
        return Response(context, status=context["status"])
    ...
    @action(detail=True, methods=["put"], url_path="questions/(?P<question_id>[^/.]+)/answer", description="Update an answer for a question")
    def update_answer(self, request, question_id=None, *args, **kwargs):
        ...
        try:
            ...
            answer = Answer.objects.filter(question=question).first()
            if not answer:
                context.update({"status": status.HTTP_404_NOT_FOUND, "message": "Answer not found"})
            else:
    ...
    @swagger_auto_schema(
        operation_summary="List all assessment types",
        operation_description="Retrieve a list of all available assessment types.",
        responses={200: openapi.Response("Success", AssessmentTypeSerializer(many=True))}
    )
    @action(detail=False, methods=["get"], url_path="types", description="List all assessment types")
    def list_assessment_types(self, request, *args, **kwargs):
    ...
    @swagger_auto_schema(
        operation_summary="Retrieve an assessment type",
        operation_description="Retrieve details of a specific assessment type.",
        responses={200: openapi.Response("Success", AssessmentTypeSerializer)}
    )
    @action(detail=True, methods=["get"], url_path="types/(?P<type_id>[^/.]+)", description="Retrieve a specific assessment type")
    def retrieve_assessment_type(self, request, type_id=None, *args, **kwargs):
        """ Retrieve a specific assessment type """
        context = {"status": status.HTTP_200_OK}
        try:
            assessment_type = AssessmentType.objects.get(pk=type_id)
            context.update({"data": AssessmentTypeSerializer(assessment_type).data})
        except AssessmentType.DoesNotExist:
            context.update({"status": status.HTTP_404_NOT_FOUND, "message": "Assessment type not found"})
        except Exception as ex:
            context.update({"status": status.HTTP_400_BAD_REQUEST, "message": str(ex)})
    ...
    @swagger_auto_schema(
        request_body=AssessmentTypeSerializer,
        operation_summary="Create a new assessment type",
        operation_description="Create a new assessment type.",
        responses={201: openapi.Response("Created", AssessmentTypeSerializer)}
    )
    @action(detail=False, methods=["post"], url_path="types", description="Create a new assessment type")
    def create_assessment_type(self, request, *args, **kwargs):
        ...
        try:
            ...
            if serializer.is_valid():
                assessment_type = serializer.save()
                context.update({
                    "data": AssessmentTypeSerializer(assessment_type).data,
                    "message": "Assessment type created successfully"
                })
            else:
                context.update({
    ...
    @swagger_auto_schema(
        ...
        request_body=AssessmentTypeSerializer,
        operation_summary="Update an assessment type",
        operation_description="Update an existing assessment type.",
        responses={200: openapi.Response("Success", AssessmentTypeSerializer)}
    )
    @action(detail=True, methods=["put"], url_path="types/(?P<type_id>[^/.]+)", description="Update an assessment type")
    def update_assessment_type(self, request, type_id=None, *args, **kwargs):
        ...
        try:
            ...
            serializer = AssessmentTypeSerializer(assessment_type, data=request.data, partial=True)
            if serializer.is_valid():
                updated_assessment_type = serializer.save()
                context.update({
                    "data": AssessmentTypeSerializer(updated_assessment_type).data,
                    "message": "Assessment type updated successfully"
                })
            else:
                context.update({
    ...
    @swagger_auto_schema(
        operation_summary="Delete an assessment type",
        operation_description="Delete an existing assessment type.",
    ...
    @swagger_auto_schema(
        operation_summary="List all assessment types",
        operation_description="Retrieve a list of all assessment types.",
        responses={200: openapi.Response("Success", AssessmentTypeSerializer(many=True))}
    )
    @action(detail=False, methods=["get"], url_path="types", description="List all assessment types")
    def list_assessment_types(self, request, *args, **kwargs):
        assessment_types = AssessmentType.objects.all()
        return Response(AssessmentTypeSerializer(assessment_types, many=True).data, status=status.HTTP_200_OK)
    ...
    @swagger_auto_schema(
        request_body=AssessmentTypeSerializer,
        operation_summary="Partially update an assessment type",
        operation_description="Partially update an existing assessment type.",
        responses={200: openapi.Response("Success", AssessmentTypeSerializer)}
    )
    @action(detail=True, methods=["put"], url_path="types/(?P<type_id>[^/.]+)", description="Update an assessment type")
    def update_assessment_type(self, request, type_id=None, *args, **kwargs):
        try:
            ...
            return Response({"detail": "Assessment type not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def delete_assessment_type(self, request, type_id=None, *args, **kwargs):
        try:
            assessment_type = AssessmentType.objects.get(id=type_id)
            assessment_type.delete()
            return Response({"detail": "Assessment type deleted"}, status=status.HTTP_204_NO_CONTENT)
        except AssessmentType.DoesNotExist:
            return Response({"detail": "Assessment type not found"}, status=status.HTTP_404_NOT_FOUND)
        

    swagger_auto_schema(
        operation_summary="List all questions and answers",
        operation_description="Retrieve a list of all questions and answers for an assessment.",
        responses={200: openapi.Response("Success", QuestionSerializer(many=True))}
    )
    @action(detail=True, methods=["get"], url_path="questions", description="List all questions in an assessment")
    def list_questions(self, request, *args, **kwargs):
    ...
    @action(detail=True, methods=["post"], url_path="questions", description="Create a new question in an assessment")
    ...
    @action(detail=True, methods=["put"], url_path="questions/(?P<question_id>[^/.]+)", description="Update a question")
    def update_question(self, request, question_id=None, *args, **kwargs):
        try:
            question = Question.objects.get(id=question_id, assessment=self.get_object())
    ...
    @action(detail=True, methods=["delete"], url_path="questions/(?P<question_id>[^/.]+)", description="Delete a question")
